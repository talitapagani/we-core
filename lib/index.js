...
we.bootstrap = function bootstrap(configOnRun, cb) {
  ...
  async.series([
    ...
    function bindRoutes(next) {
      ...
      we.router.bindRoutes(we, next);

      next();
    },
    function bindResponseMiddleware(next) {
      // bind responses
      we.express.use(function (req, res) {
        if (res.locals.responseName && res[res.locals.responseName]) {
          res[res.locals.responseName]();
        } else {
